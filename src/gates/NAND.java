...
public class NAND {
    ...
    private float cost(float a, float b, float c){
        ...
        float result = 0f;
        for (int i = 0; i< nand_train.length;i++){
            int x1 = nand_train[i][0];
            int x2 = nand_train[i][1];
            float y = sigmoid(a*x1+b*x2+c);
            float d = y - nand_train[i][2];
            float cost = d*d;
            result += cost;
            //System.out.printf("Actual: %.3f, Expected: %d\n",y,testData[i][1]);
        }
        result /= nand_train.length;
        this.cost = result;
        return result;
    }
